...
public aspect DiskProblems {
    ...
    pointcut openFile(): 
        call(java.io.FileInputStream.new(..) throws IOException+) ||
        call(java.io.RandomAccessFile.new(..) throws IOException+);
    ...
    Object around() : openFile() && testingThread() {
        ...
        if (ctx.getRandom().nextInt(100) < 25) {
            Rethrow.rethrow(new IOException("Simulated I/O exception on file opening."));
        }
        return proceed();
    }
