...
/**
 * Static utility methods for generating random strings and character sequences.
 */
public final class RandomStrings {
  /**
   * A random-length (including empty) string of ASCII characters
   * between 'a' and 'z'. Maximum length of the returned string is undefined and
   * can vary, be prepared for anything.
   */
  public static String randomAsciiString(Random r) {
    return randomCharString(r, 'a', 'z', 10);
  }

  /**
   * A random length string of characters between <code>min</code> and <code>max</code>
   * character codes. The character codes are not verified or checked for unicode
   * consistency.
   * 
  ...
  public static String randomCharString(Random r, char min, char max, int maxLength) {
    ...
    for (int i = 0; i < length; i++) {
      buffer[i] = (char) RandomInts.randomIntBetween(r, min, max);
    }
    return new String(buffer);    
  }

  /**
   * Returns a random string, including full unicode range. Maximum length of
   * the returned string is undefined and can vary, be prepared for anything.
   */
  ...
  /** 
   * Returns a random string that will have the given UTF-8 representation byte length. 
   */
  public static String randomUnicodeStringOfUTF8Length(Random r, int length) {
  ...
  /**
   * Fills the provided char[] with valid random unicode code unit sequence.
   */
  private static void randomUnicodeCharacters(Random r, char [] chars, int offset, int length) {
    int i = offset;
  ...
  /**
   * @see #randomRealisticUnicodeString(Random)
   */
  ...
  /**
   * Returns a random string of length between 0-20 codepoints, all codepoints
